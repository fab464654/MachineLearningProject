...
def my_pca(data, train_labels, test_data, test_labels, training_phase, N, goal_variance, show_plot, show_comparison):
...
def test_my_pca(first_N_eigenvectors, projected_images, train_labels, test_data, test_labels, m, show_updates):
    ...
    # 7. I reconstruct the faces and make an imshow of the original against the reconstructed one in the first 5 images!
...
def my_pca_tuning(increment_range, increment, trainFeatures, trainLabels, testFeatures, testLabels, show_updates):
    ...
    while actualVariance < finalVariance:
        ...
        _, N, accuracy = my_pca(trainFeatures, trainLabels, testFeatures, testLabels,
    ...
    df.to_csv('csvLogs/PCA_accuracy.csv', index=False)
